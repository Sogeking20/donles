(function ($) {
  ...
  /*---------- 01. On Load Function ----------*/
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        let addToCartButtons = document.querySelectorAll('.btn-shopping');

        addToCartButtons.forEach(button => {
            button.addEventListener('click', addToCart);
        });

        // Закрытие сайдбара корзины
        document.querySelector('.sideMenuCls').addEventListener('click', function () {
            document.querySelector('.sidemenu-cart').classList.remove('active');
        });

        // Делегирование событий для удаления товара из корзины
        let cartList = document.querySelector('.woocommerce-mini-cart.cart_list');
    ...
    function updateCartBadge() {
        let cartItems = document.querySelectorAll('.woocommerce-mini-cart.cart_list li.mini_cart_item');
        let cartBadge = document.querySelector('.icon-btn.sideMenuToggler .badge');
        if (cartItems.length > 0) {
            cartBadge.textContent = cartItems.length;
            cartBadge.style.display = 'block'; // Показываем бейдж, если есть товары в корзине
        } else {
            cartBadge.textContent = '0';
            cartBadge.style.display = 'none'; // Скрываем бейдж, если корзина пуста
        }
    }
    ...
    function addToCart(e) {
        ...
        // Получаем наименование товара и его цену
        let productName = this.closest('.event-list').dataset.productName;

        let productPrice = this.parentElement.querySelector('.price-cart').innerText;

        // Создаем HTML для нового товара в корзине
        let newCartItem = `
    <li class="woocommerce-mini-cart-item mini_cart_item">
        <a href="#" class="remove remove_from_cart_button">X</a>
        <a href="#" style="color: #288053!important">
            <img src="assets/img/event/event_2_2.jpg" alt="${productName}">
            ${productName}
        </a>
    ...
    `;
        ...
        let cartList = document.querySelector('.woocommerce-mini-cart.cart_list');
        cartList.innerHTML += newCartItem;
